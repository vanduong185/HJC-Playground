...
exports.get_shared_project = (req, res, next) => {
  ...
  if (options.project_id) {
    ...
    if (options.keyword == null) {
      ...
      user_id = req.params.user_id;
      query_str = 'SELECT DISTINCT p.project_id, p.author_id, u.email as author_email, p.project_name, sp.viewer_id FROM shared_projects sp JOIN projects p ON sp.project_id = p.project_id JOIN users u ON p.author_id = u.user_id WHERE sp.viewer_id = ? AND p.project_name LIKE ?';
